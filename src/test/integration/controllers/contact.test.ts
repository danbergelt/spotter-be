...
describe('contact form', () => {
  ...
  it('errors out name is not present', async () => {
    ...
    assert.ok(!res.body.success);
    assert.equal(res.body.error, 'Invalid name - must be non-empty string');
  });
  ...
  it('errors out email is not present', async () => {
    ...
    assert.ok(!res.body.success);
    assert.equal(res.body.error, 'Invalid email - must be non-empty string');
  });
  ...
  it('errors out subject is not present', async () => {
    ...
    assert.ok(!res.body.success);
    assert.equal(res.body.error, 'Invalid subject - must be non-empty string');
  });
  ...
  it('errors out if message is not present', async () => {
    ...
    assert.ok(!res.body.success);
    assert.equal(res.body.error, 'Invalid message - must be non-empty string');
  });
});
